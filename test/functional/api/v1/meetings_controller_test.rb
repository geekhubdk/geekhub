...
class Api::V1::MeetingsControllerTest < ActionController::TestCase
  ...
  test 'should get index' do
    ...
    assert !assigns[:meetings].any?{|m| m.starts_at<Time.now}
  end
  ...
  test 'should get index with organizer filter' do
    ...
    assert_response :success
    assert_not_nil assigns[:meetings]
    assert assigns[:meetings].any?{|m| m.organizer == 'ONUG'}
    assert !assigns[:meetings].any?{|m| m.organizer != 'ONUG'}
  end
  ...
  test 'should get index with organizer filter array' do
    ...
    assert_not_nil assigns[:meetings]
    assert assigns[:meetings].any?{|m| m.organizer == 'ONUG'}
    assert assigns[:meetings].any?{|m| m.organizer == 'ANUG' }
    assert !assigns[:meetings].any?{|m| m.organizer != 'ONUG' && m.organizer != 'ANUG'}
  end
  ...
  test 'should get index with location filter' do
    ...
    assert_response :success
    assert_not_nil assigns[:meetings]
    assert assigns[:meetings].any?{|m| m.city.name == 'Aarhus'}
    assert !assigns[:meetings].any?{|m| m.city.name != 'Aarhus'}
  end
  ...
  test 'should get index with location filter array' do
    ...
    assert_not_nil assigns[:meetings]
    assert assigns[:meetings].any?{|m| m.city.name == 'Aarhus'}
    assert assigns[:meetings].any?{|m| m.city.name == 'Odense'}
    assert !assigns[:meetings].any?{|m| m.city.name != 'Odense' && m.city.name != 'Aarhus'}
  end
